...
void main()
{
	...
	vec3 lightDirection = normalize(position - light.position.xyz);
	float lightDistance = length(position - light.position.xyz);
	...
	vec3 halfwayDirection = normalize(-lightDirection - cameraViewDirection);
	...
	float lightMask = (positive_dot(normal, halfwayDirection) * positive_dot(normal, -lightDirection)) / (dot(-cameraViewDirection, halfwayDirection) + EPS);
	float maskingValue = min(1.0f, min(2.0f * viewMask, 2.0f * lightMask));

	vec3 brdfValue = diffuseColour + ((normalDistribution * fresnelValue * maskingValue) / ((4 * positive_dot(normal, -cameraViewDirection) * positive_dot(normal, -lightDirection)) + EPS));

	vec3 diffuse = (brdfValue * light.diffuseColour.rgb * positive_dot(normal, -lightDirection)) / pow(lightDistance, 2.0f);
