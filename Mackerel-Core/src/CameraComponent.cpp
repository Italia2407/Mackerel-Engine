...
Eigen::Matrix4f CameraComponent::GetCameraViewMatrix() const
{
	...
	Eigen::Vector3f cameraZAxis = m_FrontDirection.normalized();
	Eigen::Vector3f cameraXAxis = cameraZAxis.cross(m_UpDirection).normalized();
	...
	Eigen::Matrix4f cameraViewMatrix = Eigen::Matrix4f::Identity(); {
		...
		cameraViewMatrix.coeffRef(0, 3) = -cameraXAxis.dot(m_Position);
		...
		cameraViewMatrix.coeffRef(1, 3) = -cameraYAxis.dot(m_Position);
		...
		cameraViewMatrix.coeffRef(2, 3) = -cameraZAxis.dot(m_Position);
	}
...
Eigen::Matrix4f ProjectionCamera::GetProjectionMatrix() const
{
